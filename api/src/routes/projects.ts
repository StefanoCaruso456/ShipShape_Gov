...
function getProjectOrderByClause(
  ...
  if (sortField === 'title') {
    return `${tableAlias}.title ${sortDir}`;
  }

  return `${tableAlias}.${sortField} ${sortDir}`;
}
...
router.get('/', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const orderByClause = getProjectOrderByClause(sortField, sortDir, 'vp');
    ...
    let query = `
      ...
      LEFT JOIN document_associations prog_da ON prog_da.document_id = vp.id AND prog_da.relationship_type = 'program'
      LEFT JOIN project_relationship_counts prc ON prc.project_id = vp.id
      LEFT JOIN project_sprint_status pss ON pss.workspace_id = vp.workspace_id AND pss.project_id = vp.id::text
    `;
