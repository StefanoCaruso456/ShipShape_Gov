...
- Current verification for this repo state:
  ...
  - `pnpm fleetgraph:verify-requirements` ran successfully for the current repo state
- A fresh local manual FleetGraph trace capture was not rerun in this workspace because no local Ship API was listening on `http://localhost:3000` and the API test harness had no reachable PostgreSQL on `127.0.0.1:5432`; the trace links above are the captured evidence bundle currently recorded in [summary.md](audit-results/fleetgraph-evidence/summary.md).
- Public deployment verification should be rerun after merge/deploy; see the latest status in [summary.md](audit-results/fleetgraph-requirements/summary.md).
