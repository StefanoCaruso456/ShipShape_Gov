# GFA Week 4: ShipShape Context

Source PDF: `/Users/stefanocaruso/Desktop/Gauntlet/GFA_Week_4-ShipShape.pdf`
...
- Use a bundle visualization tool (for example `rollup-plugin-visualizer`, `vite-bundle-analyzer`, or `source-map-explorer`) to generate a treemap of the bundle
...
| 4. | ___ ms | ___ ms | ___ ms |
| 5. | ___ ms | ___ ms | ___ ms |
...
| --- | --- | --- | --- |
| Load main page | ___ | ___ ms | Yes / No |
| View a document | ___ | ___ ms | Yes / No |
| List issues | ___ | ___ ms | Yes / No |
| Load sprint board | ___ | ___ ms | Yes / No |
| Search content | ___ | ___ ms | Yes / No |
...
- Pick one user action (for example creating an issue) and trace it from the React component through the API route to the database query and back.
