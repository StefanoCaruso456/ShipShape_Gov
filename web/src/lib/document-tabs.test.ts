...
describe('getTabsForDocument', () => {
  it('returns planning sprint tabs when sprint status is planning', () => {
...
describe('resolveTabLabels', () => {
  ...
  it('resolves dynamic labels with counts', () => {
    const tabs = getTabsForDocumentType('program');
    const resolved = resolveTabLabels(tabs, { ...mockDocument, document_type: 'program' }, { issues: 5, weeks: 3, projects: 2 });
    ...
    const projectsTab = resolved.find(t => t.id === 'projects');
    expect(projectsTab?.label).toBe('Projects (2)');
  });
