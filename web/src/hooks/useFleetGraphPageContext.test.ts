...
import type { ActiveWeeksResponse } from "@/hooks/useWeeksQuery";
import {
  buildIssueSurfacePageContext,
  buildMyWeekPageContext,
  buildProjectsPageContext,
} from "./useFleetGraphPageContext";
...
const stalledIssueSurfaceIssues: Issue[] = issueSurfaceIssues.map((issue) =>
  issue.id === "issue-1"
    ? {
        ...issue,
        updated_at: "2026-03-16T02:00:00.000Z",
...
const activeWeeksResponse: ActiveWeeksResponse = {
  weeks: [
    {
      id: "week-3",
      name: "Week 3",
  ...
  days_remaining: 3,
  sprint_start_date: "2026-03-17T00:00:00.000Z",
  sprint_end_date: "2026-03-23T00:00:00.000Z",
};
...
describe("buildMyWeekPageContext", () => {
  ...
  it("surfaces top attention projects with sprint routes and execution detail", () => {
    const context = buildMyWeekPageContext(
      "/my-week?week_number=12",
      activeView,
      baseMyWeek,
    );
    ...
    expect(context.summary).toContain(
      "Core API has 3 tracked issues and none are started yet.",
    );
  ...
  it("falls back to the project document when sprint scope is not yet linked", () => {
    ...
    const context = buildMyWeekPageContext(
      "/my-week?week_number=13",
      activeView,
      futureWeek,
    );

    expect(context.summary).toContain(
      "Core API is assigned for Week 13, but no sprint issues are linked yet.",
    );
    expect(context.items).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
...
describe("buildIssueSurfacePageContext", () => {
  ...
  it("builds issue-surface execution context from visible issue rows", () => {
    ...
    expect(context.summary).toContain(
      "API Platform has visible delivery risk from stale work.",
    );
    expect(context.metrics).toEqual(
      expect.arrayContaining([
    ...
    expect(context.items).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          route: "/documents/issue-4",
          detail: expect.stringContaining("Cut candidate"),
        }),
        expect.objectContaining({
          label: "Week 3",
    ...
    expect(context.actions).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          route: "/documents/issue-4",
          intent: "prioritize",
          reason: expect.stringContaining("Safer to move out"),
        }),
        expect.objectContaining({
  ...
  it("surfaces stalled active work when an in-progress issue goes stale", () => {
    const context = buildIssueSurfacePageContext(
      "/documents/program-1/issues",
      {
        type: "program",
      ...
      stalledIssueSurfaceIssues,
      issueSurfaceProjects,
    );

    expect(context.summary).toContain("has active work that looks stalled");
    expect(context.metrics).toEqual(
      expect.arrayContaining([{ label: "Stalled active", value: "1" }]),
    );
    expect(context.items).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          label: "#12 Implement core workflow",
          detail: expect.stringContaining("Stalled in progress"),
    ...
    expect(context.actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          route: "/documents/issue-1",
          intent: "follow_up",
          reason: expect.stringContaining("looks stalled"),
        }),
      ]),
    );
  });
  ...
  it("surfaces explicit blocker evidence when issue iterations show unresolved blockers", () => {
    ...
    expect(context.summary).toContain(
      "has explicit blocker evidence on this issues surface",
    );
    expect(context.metrics).toEqual(
      expect.arrayContaining([
      ...
      expect.arrayContaining([
        expect.objectContaining({
          ...
          detail: expect.stringContaining(
            "Blocker: Waiting on API review from platform team",
          ),
          route: "/documents/issue-1",
        }),
      ]),
...
describe("buildProjectsPageContext", () => {
  ...
  it("builds scoped project-tab context with attention, review, and after-start signals", () => {
    const scopedIssues: Issue[] = issueSurfaceIssues.map((issue) => {
      if (issue.id === "issue-1") {
        return {
          ...issue,
          state: "in_review",
    ...
    expect(context.kind).toBe("projects");
    expect(context.summary).toContain(
      "API Platform points first to Core Features",
    );
    expect(context.summary).toContain("waiting on review");
    expect(context.summary).toContain("added after sprint start");
    expect(context.metrics).toEqual(
      expect.arrayContaining([
    ...
    expect(context.items).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          detail: expect.stringContaining("Waiting on review: 1"),
        }),
        expect.objectContaining({
          label: "Performance",
          route: "/documents/project-2/issues",
          detail: expect.stringContaining("Stale open: 1"),
        }),
      ]),
    );
    expect(context.actions).toEqual(
