...
function getFallbackEntityLabel(
  activeView: FleetGraphActiveViewContext | null,
): string | null {
  if (!activeView) {
    return null;
...
function buildContextSummary(
  ...
  return [entityTitle, projectName, programName, tabLabel]
    .filter(Boolean)
    .join("  •  ");
}
...
function buildErrorGuidance(
  ...
  const workSurface = describeCurrentWorkSurface(
    activeView,
    turn.pageContext,
    result,
  );
  ...
  if (isCommentWriteFailure && isForbidden) {
    return {
      issueDetected: `FleetGraph could analyze ${workSurface}, but Ship rejected the approved follow-up comment before it could be posted.`,
      rootCause:
      ...
      impact: `The current page stays unchanged, so the team does not get the follow-up comment or escalation FleetGraph tried to add for ${workSurface}.`,
      immediateFix:
  ...
  return {
    ...
    impact: `You can still inspect ${workSurface}, but FleetGraph could not finish this answer or action cleanly on the current page.`,
    immediateFix:
      "Retry the question once. If it fails again, avoid mutating actions on this page until the backend path is verified.",
    shortTermFix:
...
function getPromptSurface(
  ...
  if (
    pageContext?.kind === "issues" ||
    pageContext?.kind === "issue_surface" ||
    activeView?.tab === "issues"
  ) {
    return "project_issues";
  }

  if (activeView?.entity.type === "program") {
    return "program";
  }
  ...
  if (pageContext?.kind === "programs" || pageContext?.kind === "dashboard") {
    return "program";
  }

  if (
    pageContext?.kind === "team_directory" ||
...
function buildMyWeekStarterPrompts(
  pageContext: FleetGraphPageContext | null,
): string[] {
  const projectSignals = getPageContextMetricValue(
    pageContext,
    "Project signals",
  );
  ...
  return dedupePrompts([
    ...
    weeklyPlan && weeklyPlan !== "Submitted"
      ...
      : null,
    dailyUpdates &&
    !dailyUpdates.startsWith("5/") &&
    !dailyUpdates.startsWith("4/")
...
function buildIssueSurfaceStarterPrompts(
  pageContext: FleetGraphPageContext | null,
): string[] {
...
function buildProjectListStarterPrompts(
  activeView: FleetGraphActiveViewContext | null,
  pageContext: FleetGraphPageContext | null,
): string[] {
  const waitingOnReview = getPageContextMetricNumber(
    pageContext,
    "Waiting on review",
  );
  const addedAfterSprintStart = getPageContextMetricNumber(
    pageContext,
    "Added after sprint start",
  );
  const needsAttention = getPageContextMetricNumber(
    pageContext,
    "Needs attention",
  );
  const topAttentionProject = getPageContextMetricValue(
    pageContext,
    "Top attention project",
  );
  const isProgramScoped = activeView?.entity.type === "program";

  return dedupePrompts([
    "Which project needs attention first?",
    waitingOnReview && waitingOnReview > 0
      ? "Where are we waiting on review or approval?"
      : null,
    addedAfterSprintStart && addedAfterSprintStart > 0
      ? "What was added after sprint start?"
      : null,
    needsAttention && needsAttention > 1
      ? isProgramScoped
        ? "Is the risk concentrated in one project or spread across the program?"
        : "Is the risk concentrated in one project or spread across the list?"
      : null,
    topAttentionProject
      ? `Should I open ${topAttentionProject} first?`
  ...
  switch (getPromptSurface(activeView, pageContext)) {
    ...
    case "project":
      if (pageContext?.kind === "projects") {
        return buildProjectListStarterPrompts(activeView, pageContext);
      }
      return dedupePrompts([
...
function inferPromptTheme(turn: FleetGraphChatTurn): FleetGraphPromptTheme {
  ...
  if (question.includes("review") || question.includes("approval")) {
    return "review";
  }
  if (
    question.includes("added after sprint start") ||
    question.includes("after sprint start")
  ) {
    return "scope";
  }
  ...
  if (
    ...
    return corpus.includes("review") && !corpus.includes("blocked")
      ? "review"
      : "blockers";
  }
...
function getPromptSurfaceForTurn(
  turn: FleetGraphChatTurn,
): FleetGraphPromptSurface {
...
function getFollowUpPrompts(turn: FleetGraphChatTurn): string[] {
  ...
  const prompts: string[] = (() => {
    switch (theme) {
      ...
      case "risk":
        ...
        if (surface === "program" || surface === "project") {
          return [
            "Which project needs attention first?",
            "Is the risk mostly review wait, after-start scope change, or stale open work?",
      ...
      case "scope":
        ...
        if (surface === "program" || surface === "project") {
          return [
            "Which project picked up the most after-start work?",
      ...
      case "review":
        if (surface === "program" || surface === "project") {
          return [
            "Which project has the biggest review queue?",
            "Is this wait concentrated in one project or spread across several?",
            "Which project should I open to clear review wait first?",
            "Is this review wait blocking higher-value work?",
          ];
        }
  ...
  return dedupePrompts(
    prompts.filter(
      (prompt) => prompt.trim().toLowerCase() !== normalizedQuestion,
    ),
  );
}

function toRouteActionLabel(label: string): string {
  return /^(Open|Create|Write|Complete)\b/.test(label)
    ? label
    : `Open ${label}`;
}
...
function isRiskClusterRouteAction(
...
function isCutCandidateRouteAction(
  action: FleetGraphPageContextAction,
): boolean {
  const corpus = [action.label, action.reason ?? ""].join(" ").toLowerCase();
...
function getRouteActions(
  ...
  const pageContextWithActions =
    pageContext as FleetGraphPageContextWithActions | null;
  ...
  const deduped = candidates.filter(
    (candidate): candidate is FleetGraphRouteAction => {
  ...
  deduped.sort((left, right) => {
    const questionBoostDelta =
      getRouteActionQuestionBoost(right, turn.question) -
      getRouteActionQuestionBoost(left, turn.question);
  ...
  return deduped
    .slice(0, 4)
    .map(({ source: _source, order: _order, ...action }) => action);
}
...
export function FleetGraphOnDemandPanel({
  ...
  const latestCompletedTurnRecord = useMemo(
    () =>
      [...turns]
        .reverse()
        .find(
          (turn) => turn.status === "completed" || turn.status === "error",
        ) ?? null,
    [turns],
  );
  ...
  const reportFeedback = useCallback(
    (event: FleetGraphFeedbackEventRequest) => {
  ...
  const handleRouteActionClick = useCallback(
    (
      ...
      reportFeedback({
        ...
        answer_mode: getAnswerMode(
          turn.result,
          turn.result?.activeView ?? activeView,
          turn.pageContext,
        ),
        latency_ms: turn.result?.telemetry.totalLatencyMs ?? null,
        surface: buildFeedbackSurfaceContext(
          turn.result?.activeView ?? activeView,
          turn.pageContext,
        ),
        route_action: {
  ...
  const composer = hasUsableContext ? (
    ...
    <div className="rounded-[28px] border border-white/10 bg-white/5 p-4 shadow-inner shadow-black/20">
      ...
      <p className="mt-3 text-xs leading-5 text-muted">
        FleetGraph should use the page you are on as context. If this still
        appears, the current surface is missing page-context wiring and needs a
        follow-up fix.
      </p>
    </div>
  );
          ...
          >
            {turns.length === 0 ? (
              ...
              turns.map((turn) => {
                ...
                const errorGuidance = buildErrorGuidance(
                  result,
                  turn,
                  activeView,
                );
                                ...
                                <div className="rounded-2xl border border-red-400/20 bg-black/15 px-3 py-3">
                                  ...
                                  <ul className="mt-2 space-y-2 text-sm leading-6 text-red-100">
                                    <li>
                                      ...
                                      {errorGuidance.immediateFix}
                                    </li>
                                    <li>
                                      <span className="font-medium">
                                        Short term:
                                      </span>{" "}
                                      {errorGuidance.shortTermFix}
                                    </li>
                                    <li>
                                      <span className="font-medium">
                                        Long term:
                                      </span>{" "}
                                      {errorGuidance.longTermFix}
                                    </li>
                                  </ul>
                                </div>
                                <div>
                          ...
                          <div className="mt-4 space-y-4">
                            ...
                            {hasDerivedMetrics ? (
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                {contextMetrics.map(
                                  (
                                    metric: FleetGraphPageContext["metrics"][number],
                                  ) => (
                                    <Metric
                            ...
                            {result &&
                              result.derivedSignals.signals.length > 0 && (
                                <div className="space-y-2">
                                  <div className="text-[11px] uppercase tracking-[0.18em] text-muted">
                                    What FleetGraph saw
                                  </div>
                                  <div className="space-y-2">
                                    {result.derivedSignals.signals
                                      .slice(0, 3)
                                      .map((signal) => (
                                        <SignalItem
                            ...
                            {result?.reasoning?.evidence &&
                              result.reasoning.evidence.length > 0 && (
                                <div className="rounded-2xl border border-white/10 bg-black/20 px-4 py-4">
                                  <div className="text-[11px] uppercase tracking-[0.18em] text-muted">
                                    Grounding evidence
                                  </div>
