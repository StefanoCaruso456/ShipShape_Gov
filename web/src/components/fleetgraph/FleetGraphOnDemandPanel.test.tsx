...
vi.mock("@/lib/fleetgraph", () => ({
  invokeFleetGraphOnDemand: (request: unknown) =>
    mockInvokeFleetGraphOnDemand(request),
  reportFleetGraphFeedback: (request: unknown) =>
    mockReportFleetGraphFeedback(request),
  resumeFleetGraphOnDemand: (request: unknown) =>
    mockResumeFleetGraphOnDemand(request),
}));
  ...
  actions: [
    {
      ...
      intent: "prioritize",
      reason:
        "#14 carries the strongest business value signal on this tab. Business value 87/100.",
...
const projectsSurfacePageContext: FleetGraphPageContext = {
  kind: "projects",
  ...
  metrics: [
    ...
    { label: "Top attention project", value: "Core Features" },
  ],
  items: [
    {
      label: "Core Features",
  ...
  actions: [
    {
      label: "Open top attention Core Features",
      route: "/documents/project-1/issues",
      intent: "prioritize",
      reason:
        "Core Features has 4 open issues, 1 issue waiting on review, and 2 issues added after sprint start.",
      owner: "stefano caruso",
    },
    {
      label: "Open review queue Core Features",
      route: "/documents/project-1/issues?fleetgraph_focus=review",
      intent: "follow_up",
...
describe("FleetGraphOnDemandPanel", () => {
  ...
  it("opens the drawer, sends a typed question, and renders the returned answer", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Send FleetGraph message" }),
    );
    ...
    expect(
      await screen.findByText("Grounded execution guidance"),
    ).toBeInTheDocument();
    expect(screen.getAllByText("Needs action").length).toBeGreaterThan(0);
    expect(
      screen.getAllByText(/FleetGraph sees clear execution drift/).length,
    ).toBeGreaterThan(0);
    expect(screen.getByText("Recommended next step")).toBeInTheDocument();
    expect(screen.getByText("What FleetGraph saw")).toBeInTheDocument();
    expect(
      screen.getByText(/API Platform - Core Features/),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByRole("button", {
        name: "Is the risk coming from scope, blockers, or capacity?",
      }),
    ).toBeInTheDocument();
  });
  ...
  it("uses page context on non-sprint pages instead of showing an unavailable state", async () => {
    ...
    mockInvokeFleetGraphOnDemand.mockResolvedValue({
        ...
        signals: [
          {
            ...
            evidence: [
              "Visible issues are still mixed.",
              "No named blocker is present yet.",
            ],
      ...
      reasoning: {
        ...
        recommendedNextStep:
          "Open the program that looks most active or least clear so you can inspect its projects, issues, and current sprint.",
        confidence: "high",
      },
      reasoningSource: "deterministic",
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: "Which project needs attention first?",
      }),
    );
    ...
    expect(
      await screen.findByText(
        "Programs shows 5 active programs in this workspace.",
      ),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("link", { name: "Open API Platform" }),
    ).toHaveAttribute(
      "href",
  ...
  it("renders scoped issue-surface answers as execution guidance instead of generic page guidance", async () => {
    ...
    expect(
      screen.getByRole("button", {
        name: "What, if anything, still needs triage on this tab?",
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Which issues are stale or stuck?" }),
    ).toBeInTheDocument();
    fireEvent.click(
      screen.getByRole("button", {
        name: "What, if anything, still needs triage on this tab?",
    ...
    expect(
      screen.getByRole("link", { name: "Open risk cluster Week 3" }),
    ).toHaveAttribute("href", "/documents/week-3/issues");
    expect(
      screen.getByText("Suggested follow-up questions"),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", {
        name: "Which status cluster is carrying the most unfinished risk?",
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "Open highest-impact #14" }),
    ).toHaveAttribute("href", "/documents/issue-3");
    expect(screen.queryByText("Page guidance")).not.toBeInTheDocument();
  });

  it("offers project-list starter prompts that match review and after-start change signals", async () => {
    mockUseFleetGraphActiveView.mockReturnValue({
      entity: {
        id: "program-1",
        type: "program",
    ...
    mockUseFleetGraphPageContext.mockReturnValue(projectsSurfacePageContext);
    mockInvokeFleetGraphOnDemand.mockResolvedValue({
      ...baseResponse,
      activeView: {
        entity: {
      ...
      reasoning: {
        answerMode: "launcher",
        summary:
          "Core Features needs attention first. Needs attention. 4 open issues. Waiting on review: 1. Added after sprint start: 2. Owner: stefano caruso.",
        evidence: [
          "Needs attention: 2",
        ...
        whyNow:
          "I ranked the visible projects using open work, review wait, after-start additions, stale movement, ownership clarity, and business value from the current projects surface.",
        recommendedNextStep:
          "Open top attention Core Features. Then inspect the project issues view to see whether the real drag is review wait, untouched scope, or stale open work.",
        confidence: "high",
      },
      reasoningSource: "deterministic",
      terminalOutcome: "quiet",
    ...
    expect(
      screen.getByRole("button", {
        name: "Where are we waiting on review or approval?",
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", {
        name: "What was added after sprint start?",
      }),
    ).toBeInTheDocument();
  });

  it("shows project-list follow-up prompts that stay aligned with review-queue answers", async () => {
    mockUseFleetGraphActiveView.mockReturnValue({
      entity: {
        id: "program-1",
        type: "program",
    ...
    mockInvokeFleetGraphOnDemand.mockResolvedValue({
      ...
      reasoning: {
        answerMode: "launcher",
        summary:
          "Review wait is most visible in Core Features. Needs attention. 4 open issues. Waiting on review: 1. Added after sprint start: 2. Owner: stefano caruso. This surface shows review queue pressure directly; approval state beyond that is only inferred here.",
        evidence: [
          "Waiting on review: 1 issue",
          "Top attention project: Core Features",
        ],
        whyNow:
          "I used the visible in-review queue on this tab as the strongest review signal. Approval state beyond that is only inferred here when the page lacks a clearer decision owner.",
        recommendedNextStep:
          "Open review queue Core Features. Then inspect the project issues view to see whether the wait is really review queue pressure or a missing owner decision.",
        confidence: "high",
      },
      reasoningSource: "deterministic",
      terminalOutcome: "quiet",
    ...
    expect(
      await screen.findByRole("link", {
        name: "Open review queue Core Features",
      }),
    ).toHaveAttribute(
      "href",
      "/documents/project-1/issues?fleetgraph_focus=review",
    );
    expect(
      screen.getByRole("button", {
        name: "Which project has the biggest review queue?",
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", {
        name: "Which project should I open to clear review wait first?",
      }),
    ).toBeInTheDocument();
  });
  ...
  it("promotes a cut candidate route for cut questions on issue surfaces", async () => {
    ...
    expect(
      await screen.findByRole("link", { name: "Review cut candidate #15" }),
    ).toHaveAttribute("href", "/documents/issue-15");
    expect(
      screen.getAllByText(
        /Safer to move out than the active or higher-value work/,
      ).length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByRole("button", {
        name: "What can move out without hurting the highest-value work?",
      }),
    ).toBeInTheDocument();
  });
  ...
  it("keeps the risk-cluster route featured for risk questions even when a cut candidate is present", async () => {
    const riskContext: FleetGraphPageContext = {
      ...
      actions: [
        ...
        },
        {
          ...
          intent: "prioritize",
          reason:
            "#19 carries the strongest business value signal on this tab. Business value 64/100.",
        },
      ],
    };
    ...
    mockInvokeFleetGraphOnDemand.mockResolvedValue({
      ...
      reasoning: {
        answerMode: "execution",
        summary:
          "Week 2 is the clearest risk cluster on this tab. 10 issues are still not started.",
        evidence: ["Not started: 10", "Risk cluster: Week 2"],
        whyNow:
          "This answer is grounded in the visible issues on the current tab.",
        recommendedNextStep:
  ...
  it("still shows an unavailable state only when neither active view nor page context exists", () => {
    ...
    expect(
      screen.getAllByText(/FleetGraph could not derive current page context/)
        .length,
    ).toBeGreaterThan(0);
  ...
  it("supports HITL decisions from the drawer", async () => {
    ...
    mockInvokeFleetGraphOnDemand.mockResolvedValue({
      ...
      proposedAction: {
        ...
        draftComment:
          "Can you post a same-day status update and confirm the next checkpoint?",
  ...
  it("translates writeback failures into structured execution guidance", async () => {
    ...
    expect(
      screen.getByText(
        /Ship rejected the approved follow-up comment before it could be posted/i,
      ),
    ).toBeInTheDocument();
