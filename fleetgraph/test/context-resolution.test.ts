...
describe("FleetGraph non-week context resolution", () => {
  ...
  it("answers from page context when there is no sprint entity but the current page snapshot is available", async () => {
    ...
    expect(result.reasoning?.summary).toContain(
      "Programs shows 5 active programs",
    );
          ...
          pageContext: {
            ...
            items: [
              ...
              },
              {
                ...
                detail:
                  "State: todo • Week: Week 3 • Owner: stefano caruso • Updated 4d ago",
            ...
            actions: [
              {
                ...
                intent: "prioritize",
                reason:
                  "Week 3 holds 3 open issues with 2 issues still not started.",
              },
            ],
          },
        },
  ...
  it("answers highest-impact issue questions from issue-surface business value signals", async () => {
            ...
            items: [
              {
                label: "#14 Expand test coverage",
                detail:
                  "Highest impact • Project: Performance • Business value: 87/100 • Drivers: ROI 5/5 + Growth 5/5",
                route: "/documents/issue-3",
              },
            ],
            actions: [
    ...
    expect(result.reasoning?.answerMode).toBe("execution");
    expect(result.reasoning?.summary).toContain(
      "#14 is the highest-impact visible issue",
    );
    expect(result.reasoning?.summary).toContain("87/100");
    expect(result.reasoning?.recommendedNextStep).toContain(
      "Open highest-impact #14",
    );
  });
  ...
  it("answers triage questions with the riskiest untouched cluster instead of generic cut guidance", async () => {
          ...
          pageContext: {
            ...
            items: [
              ...
              },
              {
                label: "#14 Expand test coverage",
                detail:
                  "Highest impact • Project: Performance • Business value: 87/100 • Risk: not started inside Week 3",
                route: "/documents/issue-3",
              },
            ],
            actions: [
    ...
    expect(result.reasoning?.answerMode).toBe("execution");
    expect(result.reasoning?.summary).toContain(
      "Week 3 is where triage pressure is building",
    );
  ...
  it("answers blocker questions from explicit issue-surface blocker evidence", async () => {
    ...
    expect(result.reasoning?.answerMode).toBe("execution");
    expect(result.reasoning?.summary).toContain(
      "#12 Implement core workflow is the clearest visible blocker right now",
    );
    expect(result.reasoning?.summary).toContain("Oldest blocker: 4 days");
    expect(result.reasoning?.recommendedNextStep).toContain(
      "Follow up on blocker #12",
    );
  });
  ...
  it("answers stalled-work questions with a direct stalled issue instead of generic risk copy", async () => {
    ...
    expect(result.reasoning?.answerMode).toBe("execution");
    expect(result.reasoning?.summary).toContain(
      "#12 Implement core workflow is the clearest in-progress issue that looks stalled right now",
    );
    expect(result.reasoning?.recommendedNextStep).toContain(
      "Follow up on stalled #12",
    );
  });
  ...
  it("answers cut questions with named cut candidates instead of generic highest-impact guidance", async () => {
    ...
    expect(result.reasoning?.answerMode).toBe("execution");
    expect(result.reasoning?.summary).toContain(
      "If you need to cut scope, start with #15 Explore stretch improvements, then #10 Explore stretch improvements",
    );
    expect(result.reasoning?.summary).toContain("lower value than #9");
    expect(result.reasoning?.recommendedNextStep).toContain(
      "Review cut candidate #15",
    );
  });
  ...
  it("answers risk questions inside a week with the risk cluster CTA instead of a cut candidate", async () => {
    ...
    expect(result.reasoning?.answerMode).toBe("execution");
    expect(result.reasoning?.summary).toContain(
      "Week 2 is the clearest risk cluster on this tab",
    );
    expect(result.reasoning?.recommendedNextStep).toContain(
      "Open risk cluster Week 2",
    );
    expect(result.reasoning?.recommendedNextStep).not.toContain(
      "Review cut candidate #15",
    );
  });

  it("answers program project-tab attention questions with a named project instead of generic launcher copy", async () => {
    const graph = createFleetGraph();
    const runtime = createFleetGraphRuntime({
      now: () => new Date("2026-03-20T12:00:00.000Z"),
    });
    ...
    const result = await graph.invoke(
      {
        runId: "projects-surface-attention-context",
        mode: "on_demand",
        triggerType: "user_invoke",
        workspaceId: "workspace-1",
        actor: {
        ...
        prompt: {
          ...
          pageContext: {
            ...
            metrics: [
              ...
              { label: "Highest value project", value: "Performance" },
            ],
            items: [
              {
                label: "Core Features",
                detail:
                  "Needs attention • 4 open issues • Waiting on review: 1 • Added after sprint start: 2 • Not started: 1 • Owner: stefano caruso",
                route: "/documents/project-1/issues",
              },
              {
                label: "Performance",
                detail:
                  "Needs attention • 2 open issues • Stale open: 1 • Business value: 87/100 • Owner: stefano caruso",
                route: "/documents/project-2/issues",
              },
            ],
            actions: [
              {
              ...
              {
                label: "Open highest-value Performance",
                route: "/documents/project-2",
                intent: "inspect",
                reason:
                  "Performance carries the strongest business value signal on this project list at 87/100.",
                owner: "stefano caruso",
              },
            ],
          },
        ...
        trace: {
          runName: "fleetgraph-projects-surface-attention-test",
          tags: ["fleetgraph", "test", "projects-surface", "attention"],
        },
      } satisfies FleetGraphRunInput,
      createFleetGraphRunnableConfig(runtime, {
        threadId: "projects-surface-attention-context",
      }),
    );

    expect(result.status).toBe("completed");
    expect(result.stage).toBe("current_view_reasoned");
    expect(result.reasoning?.answerMode).toBe("launcher");
    expect(result.reasoning?.summary).toContain(
      "Core Features needs attention first",
    );
    expect(result.reasoning?.recommendedNextStep).toContain(
      "Open top attention Core Features",
    );
  });

  it("answers project-list review questions from visible review-queue signals", async () => {
    const graph = createFleetGraph();
    const runtime = createFleetGraphRuntime({
      now: () => new Date("2026-03-20T12:00:00.000Z"),
    });
    ...
    const result = await graph.invoke(
      {
        runId: "projects-surface-review-context",
        mode: "on_demand",
        triggerType: "user_invoke",
        workspaceId: "workspace-1",
        actor: {
          ...
          pageContext: {
            ...
            items: [
              ...
              },
              {
                label: "Performance",
                detail:
                  "In flight • 2 open issues • Business value: 87/100 • Owner: stefano caruso",
                route: "/documents/project-2/issues",
              },
            ],
            actions: [
        ...
        trace: {
          runName: "fleetgraph-projects-surface-review-test",
          tags: ["fleetgraph", "test", "projects-surface", "review"],
        },
      } satisfies FleetGraphRunInput,
      createFleetGraphRunnableConfig(runtime, {
        threadId: "projects-surface-review-context",
      }),
    );
    ...
    expect(result.reasoning?.answerMode).toBe("launcher");
    expect(result.reasoning?.summary).toContain(
      "Review wait is most visible in Core Features",
    );
    expect(result.reasoning?.whyNow).toContain("visible in-review queue");
    expect(result.reasoning?.recommendedNextStep).toContain(
      "Open review queue Core Features",
    );
  });
